            ...
            factory = { ctx ->
                TerminalView(ctx, null).apply {
                    ...
                    val session = sessionBinder.getSession(service.currentSession.value.first)
                        ?: if (Settings.working_Mode == WorkingMode.APP_SYSTEM) {
                            AppSystemSessionProvider.createAppSystemMenuSession(
                                context = ctx,
                                sessionClient = client,
                                sessionId = service.currentSession.value.first
                            ).also {
                                sessionBinder.createCustomSession(
                                    service.currentSession.value.first,
                                    it,
                                    WorkingMode.APP_SYSTEM
                                )
                            }
                        } else {
